...
if(isset($_GET["id"])){
	...
	$arr=getData($conn,$sql);
		foreach($arr as $d) {	
			...
			$spangkat2=	"($arV1[$no]-$arV1[0])^2 + ($arV2[$no]-$arV2[0])^2 + ($arV4[$no]-$arV4[0])^2 + ($arV5[$no]-$arV5[0])^2 + ($arV6[$no]-$arV6[0])^2 ";
			$arSS[$no]="$spangkat2^0.5";
...
process($conn,"delete from `tb_hasil` where id_pengujian='$id_pengujian'");
	...
	$sql="update `$tbpengujian` set `hasil_mobil`='$keterangan' where `id_pengujian`='$id_pengujian'";
process($conn,$sql);	
...
$gab4.="<tr bgcolor='$color'>
		...
		$ket="";
		if($c1<=5000000){$leasing="CLIPAN FINANCE";}
else 		if($c1<=10000000){$leasing="ANDALAN FINANCE";}
else 		if($c1<=15000000){$leasing="OTO FINANCE";}
else 		{$leasing="BCA FINANCE";}

	$sql="INSERT INTO `tb_hasil` (`id_hasil`, `id_pengujian`, `id_mobil`, `rekapitulasi`, `bobot`, `ranking`, `keterangan`) VALUES 
	('', '$id_pengujian', '$arID[$i]', '$arS[$i]', '$arT[$i]', '$i', '$ket');";	
	process($conn,$sql);
	
	$record["id_pengujian"] = $id_pengujian;
	$record["id_mobil"] = $arID[$i];
	$record["mobil"] = getMobil($conn,$arID[$i]);
	$record["rekapitulasi"] = $arSS[$i];
	$record["bobot"] = $arT[$i];
	$record["ranking"] = $i;
	$record["keterangan"] = "Pengujian Via Android";
	
     array_push($respon["record"], $record); 
		
}
$hasil=$arNM[1]." (Bobot: ".$arT[1]."), ".$arNM[2]." (Bobot: ".$arT[2]."), ".$arNM[3]." (Bobot: ".$arT[3].")";

	$sql="update `tb_hasil` set `keterangan`='$hasil' where `id_pengujian`='$id_pengujian'";
process($conn,$sql);	

	$respon["sukses"] = 1;
	$respon["pesan"] = "$no record";
